...
export function AuditView({
  ...
  return (
    <section className="table-card">
      ...
      {logs.length === 0 ? (
        <InlineState
          title={hasActiveFilters ? 'По текущим фильтрам журнал пуст' : 'Журнал пока пуст'}
          message={
            hasActiveFilters
              ? 'Измени фильтры или выполни действия в панели, чтобы увидеть журнал.'
              : 'Выполни действия в панели, чтобы журнал начал наполняться событиями.'
          }
        />
      ) : visibleLogs.length === 0 ? (
        <InlineState
