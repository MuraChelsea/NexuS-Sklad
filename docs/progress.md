...
463. Проведен `team pending-invite integrity` pass: `TeamView` переведен на единый predicate `isPendingInvite`, чтобы stale `inviteExpiresAt` у уже активированного пользователя не искажал summary badge `Приглашения`, фильтр `INVITED`, row-status и поиск по команде.
464. Проведен `catalog import dry-run` pass: в `ProductsView` добавлен workflow `Импорт CSV` с шаблоном, textarea/file intake, preview по строкам и режимами `создать / обновить / пропустить / ошибка`; import матчит товары по `SKU`, затем `barcode`, затем по точной паре `название + единица`.
465. Для существующих товаров import теперь не только обновляет карточку через `updateProduct`, но и при необходимости делает `adjustment` до целевого `currentStock`, чтобы массовое выравнивание остатков шло через существующий audit-friendly контур.
466. После catalog import dry-run pass локально подтверждены: `apps/web npm run check` — ok, `npm run test:render` — `154 passed`, `npm run build` — ok; добавлены smoke/helper-кейсы на preview import-плана, unknown category errors, кнопку `Импорт CSV` и рендер dry-run summary в `CatalogImportModal`.
467. Проведен `catalog import atomic apply` pass: `/v1/products/import` теперь принимает готовый import-план из web dry-run и применяет его в одной транзакции, включая создание новых товаров, обновление существующих карточек и `movement.adjustment.created` для доведения остатка до `targetQty`.
468. Web import workflow переведен с последовательных `create/update/adjustment` вызовов на один atomic endpoint, поэтому массовый импорт больше не оставляет каталог в частично примененном состоянии при ошибке на середине пачки.
469. После catalog import atomic apply pass локально подтверждены: `packages/shared npm run codegen:openapi` — ok, `codegen:openapi:check` — ok, `check` — ok; `apps/api npm run check` — ok, `npm run test:smoke` — ok; `apps/web npm run check` — ok, `npm run test:contract` — ok, `npm run test:render` — `154 passed`, `npm run build` — ok.
